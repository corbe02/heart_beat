...
void OpticalFlowPose::computeOpticalFlow(const cv::Mat &prev, const cv::Mat &current)
{
    ...
    */
    ...
    if(first_time_)
    {
        ...
        cv::goodFeaturesToTrack(old_gray, points_prev_, 1000, 0.01, 50);
    ...
    drawStuff(current, good_new, cv::Scalar(0, 255, 0));
...
void OpticalFlowPose::recoverPose(const std::vector<cv::Point2f> &good_old, const std::vector<cv::Point2f> &good_new,const std::vector<bool>& dynamic)
{
    ...
    */
    ...
    {
        if (dynamic[i])
            cv::circle(current_img_, good_new[i], 6, cv::Scalar(0, 0, 255), -1);  // Red color, filled circle
        else 
            cv::circle(current_img_, good_new[i], 6, cv::Scalar(0, 255, 0), -1);  // Green color, filled circle
...
void OpticalFlowPose::drawStuff(const cv::Mat &current, std::vector<cv::Point2f> &good_new, const cv::Scalar& color)
{
    if(good_new.size() < 3)
        return;
    ...
    for( std::vector<cv::Point2f>::iterator it = good_new.begin(); it != good_new.end(); ++it)
    {
        if (rect.contains(*it)) // Verifica se il punto è dentro i limiti dell'immagine
        {
            subdiv.insert(*it);
        }
    }
